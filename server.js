  ...
  try {
    ...
    if (cheapWords.some(w => msg.includes(w)) && session.step !== "idle") {
      if (session.flights && session.flights.length > 0) {
        ...
        const link = (()=>{ const indCodes = new Set(["BLR","BOM","DEL","MAA","HYD","CCU","GOI","PNQ","COK","AMD","JAI","LKO","VNS","PAT","IXC","GAU","BBI","CBE","IXM","IXE","MYQ","TRV","VTZ","VGA","IXR","BHO","SXR","IXJ","HBX","IXG","TIR","IXL","IXZ","NAG","IDR","RPR","DED","SLV","ATQ","UDR","JDH","AGR","STV"]); const isIndia = indCodes.has(fromCode) && indCodes.has(toCode); const domain = isIndia ? "www.aviasales.in" : "www.aviasales.com"; const searchUrl = domain+"/search/"+fromCode+(session.dateStr||"")+toCode+"1"; return "https://tp.media/click?shmarker=714667&promo_id=4132&source_type=link&type=click&user_ip=d&locale=en&searchUrl="+encodeURIComponent(searchUrl)+"&sub_id=alvryn_whatsapp"; })();
